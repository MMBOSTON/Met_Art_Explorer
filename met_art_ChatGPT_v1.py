...
- Updated for wrap around text on the left navigation bar
...
class Explorer:
    ...
    def go_to_room(self, department_id, name):
        ...
        total_objects_text = f"Total Objects in {name}: {len(self.objects)}"
        wrapped_text = textwrap.fill(total_objects_text, width=30)  # Adjust width as needed
        st.sidebar.text(wrapped_text)
        self.show_room()
